...
static ssize_t nilfs_file_read_iter(struct kiocb *iocb, struct iov_iter *to)
{
	if (iocb->ki_flags & IOCB_DIRECT) {
		/*
		 * Needs synchronization with the cleaner: nilfs_iomap_ops
		 * takes the same ns_dat mi_sem that the ordinary buffered
		 * read path (nilfs_get_block()) does.
		 */
		return iomap_dio_rw(iocb, to, &nilfs_iomap_ops, NULL, 0, NULL,
				    0);
	}
...
static ssize_t nilfs_file_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	 ...
	 * delay-allocated (see nilfs_get_block()) and are only given a
	 * real disk address when the segment constructor writes them out
	 * as part of a log, which works directly on buffer_head lists
	 * (see nilfs_segctor_collect_blocks() in segment.c) rather than
	 * through iomap. Fall back to the ordinary buffered write path
	 * for O_DIRECT writes, the same way the old direct_IO() aops
	 * callback did by simply returning 0 for WRITE and letting the
	 * generic write path retry buffered.
	 */
	if (iocb->ki_flags & IOCB_DIRECT)
