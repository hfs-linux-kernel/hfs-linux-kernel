...
/*
 *  linux/fs/hfs/iomap.c
 *
 * iomap callback functions for the hfs filesystem
 */
...
static int __hfs_iomap_begin(struct inode *inode, loff_t offset,
			     loff_t length, unsigned int flags,
			     struct iomap *iomap, bool may_alloc)
{
	struct super_block *sb = inode->i_sb;
	struct hfs_sb_info *sbi = HFS_SB(sb);
	struct hfs_inode_info *hip = HFS_I(inode);
	u16 ablock, dblock, max_blocks;
	...
	ablock = div_u64(offset, sbi->alloc_blksz);

	err = hfs_map_extent(inode, ablock, may_alloc, &dblock, &max_blocks,
			     NULL);
	if (err)
		return err;

	ablock_offset = offset - (loff_t)ablock * sbi->alloc_blksz;
	ablock_bytes = (loff_t)max_blocks * sbi->alloc_blksz;

	length = min_t(loff_t, length, ablock_bytes - ablock_offset);

	/*
	 * HFS has no on-disk "unwritten extent" bit, so a clump allocated
	 * by hfs_extend_file() is never physically zeroed: blocks past
	...
	iomap->offset = offset;
	iomap->length = length;
	iomap->addr = hfs_ablock_to_phys_bytes(sb, dblock) + ablock_offset;
...
static int hfs_iomap_begin(struct inode *inode, loff_t offset,
			   loff_t length, unsigned int flags,
			   struct iomap *iomap, struct iomap *srcmap)
{
	return __hfs_iomap_begin(inode, offset, length, flags, iomap, false);
}

static int hfs_write_iomap_begin(struct inode *inode, loff_t offset,
				 loff_t length, unsigned int flags,
				 struct iomap *iomap, struct iomap *srcmap)
{
	return __hfs_iomap_begin(inode, offset, length, flags, iomap, true);
}

const struct iomap_ops hfs_iomap_ops = {
	.iomap_begin = hfs_iomap_begin,
};
 ...
 */
static int hfs_write_iomap_end(struct inode *inode, loff_t pos,
...
const struct iomap_ops hfs_write_iomap_ops = {
	.iomap_begin	= hfs_write_iomap_begin,
	.iomap_end	= hfs_write_iomap_end,
};

/*
 * hfs_iomap_cont_expand()
 *
 * Zero-extend the backing store from the current phys_size up to 'size'.
 * Used both by hfs_inode_setattr() and by hfs_file_truncate().
 */
int hfs_iomap_cont_expand(struct inode *inode, loff_t size)
{
	...
	return iomap_zero_range(inode, start, size - start, NULL,
				&hfs_write_iomap_ops, NULL, NULL);
}

/*
 * hfs_writeback_range() - map folio during writeback
 *
 * Called for each folio during writeback. If the folio falls outside
 * the current iomap, remaps by calling __hfs_iomap_begin() again.
 */
static ssize_t hfs_writeback_range(struct iomap_writepage_ctx *wpc,
				   ...
				   unsigned int len, u64 end_pos)
{
	...
	if (offset < wpc->iomap.offset ||
		...
		err = __hfs_iomap_begin(wpc->inode, offset, len, 0,
					&wpc->iomap, false);
		if (err)
			return err;
	}
...
const struct iomap_writeback_ops hfs_writeback_ops = {
	.writeback_range	= hfs_writeback_range,
	.writeback_submit	= iomap_ioend_writeback_submit,
};

/*
 * hfs_file_write_dio_end_io() - Direct I/O write completion handler
 */
static int hfs_file_write_dio_end_io(struct kiocb *iocb, ssize_t size,
...
const struct iomap_dio_ops hfs_write_dio_ops = {
	.end_io		= hfs_file_write_dio_end_io,
};

int hfs_iomap_swap_activate(struct swap_info_struct *sis,
			    struct file *file, sector_t *span)
{
	return iomap_swapfile_activate(sis, file, span, &hfs_iomap_ops);
}
