...
/*
 * buffer_head/hfsplus_get_block() based read_folio, kept for whatever
 * hasn't been converted to iomap: the B-tree metadata files
 * (hfsplus_btree_aops) and symlink content (hfsplus_symlink_aops).
 * The latter needs a_ops->write_begin/write_end too, since
 * page_symlink() in fs/namei.c calls them directly to write the link
 * target.
 */
